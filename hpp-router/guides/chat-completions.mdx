...
HPP Router supports two payment rails. Always send an API key for consumer identification.

| Rail | How to enable | When to use |
| --- | --- | --- |
| **Quota** (default) | Omit `X-Payment-Rail` | Prepaid credit on the API key |
| **Wallet (x402)** | `X-Payment-Rail: wallet` | On-chain settlement per request |

On the wallet rail, paid models typically return **`402`** until you attach a payment signature. Free providers (for example some Ollama models) are not wallet-billed even if the header is set.

See [Authentication](../authentication) for key schemes and [TypeScript SDK — Wallet](../client-sdk/typescript#wallet-x402-payment-rail) for the automatic SDK loop.
...
- **`402`** — wallet rail payment challenge (`PAYMENT-REQUIRED`). Sign and retry once.
- **`429`** — rate limit or prepaid **quota** exhaustion (not the normal wallet challenge).

See [Errors](./errors) for the full status table and envelope shapes.
