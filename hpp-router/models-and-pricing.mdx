...
HPP Router exposes models from multiple providers behind one OpenAI-compatible API. Billing is **token-based** and settled on-chain via x402 wallet (USDC).
...
- `GET /api/quota-check` — check remaining balance for the authenticated consumer.
- `GET /api/usage` — usage summary (requests, total tokens, total cost).
