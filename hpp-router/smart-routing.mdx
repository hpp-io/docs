...
<CodePanel>
<Tabs groupId="smart-routing-request" queryString>
...
If `stream: true`, basket classification is **skipped** and the request always uses the configured **streaming fallback** model. This prioritizes the stability of the streaming (SSE) pipeline. See [Streaming](./guides/streaming).
...
Billing always uses the **resolved** model's pricing and the upstream `usage` — there is no separate price for `hpprouter/auto`. The resolved model is recorded in your [usage logs](./guides/quota-and-usage).
